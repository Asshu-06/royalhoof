  ...
  return (
    <form onSubmit={handleSubmit} className="border border-[rgba(8,43,73,0.15)] rounded-xl p-4 bg-[#F4E9D2] space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><label className={lbl}>Phone *</label><input value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value.replace(/\D/g,"").slice(0,10)}))} onKeyDown={e=>{if(!/[0-9]/.test(e.key)&&!['Backspace','Delete','ArrowLeft','ArrowRight','Tab','Enter'].includes(e.key)&&!e.ctrlKey&&!e.metaKey){e.preventDefault()}}} placeholder="10-digit number" maxLength={10} inputMode="numeric" pattern="[0-9]{10}" type="tel" className={inp} />{errors.phone&&<p className="text-red-400 text-xs mt-0.5">{errors.phone}</p>}</div>
